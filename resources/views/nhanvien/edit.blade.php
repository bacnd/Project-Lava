...
@section('title')
Cập nhật nhân viên: {{ $nhanvien->hoten }}
@stop
...
<section class="panel">
    <header class="panel-heading">Cập nhật nhân viên {{ $nhanvien->hoten }}</header>
    <div class="panel-body">
        <div class="panel-body">
            ...
            <form class="form-horizontal" role="form" method="POST" action="{{ route('nhanvien.update', $nhanvien->manv) }}" enctype="multipart/form-data">
                ...
                <label for="userid" class="col-md-4 control-label">Username <span class="required">*</span></label>
                    ...
                    <select class="form-control" name="userid">
                        @foreach($users as $user)
                          <option value="{{$user->id}}" {{ $user->id == $nhanvien->userid ? "selected":"" }}>{{$user->name}}</option>
                        @endforeach
                    </select>
                    </div>
                </div>
                ...
                <label for="hoten" class="col-md-4 control-label">Họ tên <span class="required">*</span></label>
                    <div class="col-sm-6">
                        <input id="hoten" type="text" value="{{ $nhanvien->hoten }}" class="form-control" name="hoten" placeholder="Họ tên" required />
                    </div>
                </div>
                <div class="form-group col-lg-12">
                ...
                <label for="ngaysinh" class="col-md-4 control-label">Ngày sinh <span class="required">*</span></label>
                    <div class="col-sm-6">
                        ...
                        <input type="text" id="ngaysinh" value="{{ $nhanvien->ngaysinh }}" name="ngaysinh" class="form-control" placeholder="Ngày sinh" required />
                ...
                <label for="cmnd" class="col-md-4 control-label">CMND <span class="required">*</span></label>
                    <div class="col-sm-6">
                        <input id="cmnd" type="text" class="form-control" value="{{ $nhanvien->cmnd }}" name="cmnd" placeholder="CMND" required />
                    </div>
                </div>
                <div class="form-group col-lg-12">
                <label for="dienthoai" class="col-md-4 control-label">Điện thoại <span class="required">*</span></label>
                    <div class="col-sm-6">
                        <input id="dienthoai" type="text" value="{{ $nhanvien->dienthoai }}" class="form-control" name="dienthoai" placeholder="Điện thoại" required />
                    </div>
                </div>
                <div class="form-group col-lg-12">
                ...
                <label for="ngaybatdaulamviec" class="col-md-4 control-label">Ngày bắt đầu làm việc <span class="required">*</span></label>
                    <div class="col-sm-6">
                        <div data-date-viewmode="years" data-date-format="yyyy-mm-dd" data-date="<?php echo date('Y-m-d')?>"  class="input-append date dpYears">
                        <input type="text" id="ngaybatdaulamviec" value="{{ $nhanvien->ngaybatdaulamviec }}" name="ngaybatdaulamviec" class="form-control" placeholder="Ngày bắt đầu làm việc" required />
                        <span class="input-group-btn add-on">
                        <button class="btn btn-primary" type="button"><i class="fa fa-calendar"></i></button>
                        </span>
                        </div>
                    ...
                    <div class="col-sm-6">
                        <select class="form-control" name="tinhtrang">
                            @foreach($tinhtranglamviec as $ttlv)
                              <option value="{{$ttlv->matt}}" {{ $ttlv->matt == $nhanvien->matt ? "selected":"" }}>{{$ttlv->tinhtranglamviec}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-sm-6">
                        <select class="form-control" name="bangcap">
                            @foreach($bangcap as $bc)
                              <option value="{{$bc->mabc}}" {{ $bc->mabc == $nhanvien->mabc ? "selected" : "" }}>{{$bc->bangcap}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-sm-6">
                        <select class="form-control" name="phongban">
                            @foreach($phongban as $pb)
                              <option value="{{$pb->mapb}}" {{ $pb->mapb == $nhanvien->mapb ? "selected" : "" }}>{{$pb->tenphongban}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-sm-6">
                        <select class="form-control" name="chucvu">
                            @foreach($chucvu as $cv)
                              <option value="{{$cv->macv}}" {{ $cv->macv == $nhanvien->macv ? "selected" : "" }}>{{$cv->chucvu}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <label for="chucvu" class="col-md-4 control-label">Lương cơ bản <span class="required">*</span></label>
                    <div class="col-sm-6">
                        <input id="luongcoban" type="text" value="{{ $nhanvien->luongcoban }}" class="form-control" name="luongcoban" placeholder="Lương cơ bản" required />
                    </div>
                </div>
                <div class="form-group col-lg-12">
                <label for="chucvu" class="col-md-4 control-label">Trợ cấp <span class="required">*</span></label>
                    <div class="col-sm-6">
                        <input id="trocap" type="text" value="{{ $nhanvien->trocap }}" class="form-control" name="trocap" placeholder="Trợ cấp" required />
                    </div>
                </div>
                <div class="form-group col-lg-12">
                <label for="chucvu" class="col-md-4 control-label">Nghỉ phép <span class="required">*</span></label>
                    <div class="col-sm-6">
                        <input id="nghiphep" type="text" value="{{ $nhanvien->nghiphep }}" class="form-control" name="nghiphep" placeholder="Nghỉ phép" required />
                    </div>
                </div>
                <div class="form-group col-lg-12">
                <label for="ghichu" class="col-md-4 control-label">Ghi chú </label>
                    <div class="col-sm-6">
                    <textarea class="form-control" rows="5" value="{{ $nhanvien->ghichu }}" id="ghichu" name="ghichu"> </textarea>
                    </div>
                </div>
